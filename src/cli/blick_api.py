"""
This is a basic FastAPI wrapper around the blick library to provide access to a single blick
...
import blick.blick_checker

app = FastAPI()

# Globally define checker module
__blick_checker: blick.blick_checker.BlickChecker | None = None


def set_blick_checker(chk: blick.blick_checker.BlickChecker) -> None:
    """At this time we only handle one module at a time."""
    global __blick_checker
...
def prepare_blick():
    """Recalculate the initial conditions for blick"""
    __blick_checker.pre_collect()
    __blick_checker.prepare()


def checker_ok():
    """Verify that the blick object is initialized, this should always be the case"""

    # Look before you leap...not great python
    if __blick_checker is None:
        raise HTTPException(
            status_code=500,
            detail="BlickChecker is not set",
        )
    return True
...
@app.get("/blick/header")
async def get_result_header():
    """
    Return useful information about the collection of blick rule functions.

    """
    checker_ok()

    return __blick_checker.get_header()
...
def run_matched(matched_funcs, var) -> dict:
    ...
    __blick_checker.collected = matched_funcs
    __blick_checker.run_all()

    return __blick_checker.as_dict()


@app.get("/blick/all")
async def check_all() -> dict:
    """Run the whole blick ruleset (careful with timeouts)"""
    checker_ok()
...
@app.get("/blick/rule_id_re/{rule_id}")
async def check_rule_id_regular_expression(rule_id: str):
    ...
    curl "http://<your_host>:<port>/blick/rule_id/<rule_id>"

    Using Python requests library:
    import requests
    response = requests.get(f"http://<your_host>:<port>/blick/rule_id/{rule_id}")
    print(response.json())
    """
    prep_rules()
    matched_funcs = [func for func in __blick_checker.collected if re.match(rule_id, func.ruid)]
    return run_matched(matched_funcs, f"{rule_id=}")


@app.get("/blick/rule_ids/{rule_ids}")
async def check_rule_ids(rule_ids: str) -> dict:
    ...
    matched_funcs = [func for func in __blick_checker.collected if func.ruid in rule_ids]
    return run_matched(matched_funcs, f"{rule_ids=}")


@app.get("/blick/tag/{tags}")
async def check_tags(tags: str) -> dict:
    ...
    Using curl from a terminal:
    curl "http://<your_host>:<port>/blick/tags/<tags>"

    Using Python requests library:
    import requests
    response = requests.get(f"http://<your_host>:<port>/blick/tag/{tags}")
    print(response.json())
    """
    prep_rules()
    tags = [tag.strip() for tag in tags.replace(",", " ").split()]
    matched_funcs = [func for func in __blick_checker.collected if func.tag in tags]
...
async def check_level_range(
    ...
    Examples:
        ...
        Using Python requests library:
        import requests
        response = requests.get(f"http://<your_host>:<port>/blick/check_level_range?low=10&high=20")
        print(response.json())
    """

    low, high = low or -1000000, high or 1000000
    prep_rules()
    matched_funcs = [func for func in __blick_checker.collected if low <= func.level <= high]
    return run_matched(matched_funcs, f"{low=} {high=}")


@app.get("/blick/phase/{phases}")
async def check_phase(phases: str) -> dict:
    ...
    Using curl from a terminal:
    curl "http://<your_host>:<port>/blick/level_lt/level"

    Using Python requests library:
    import requests
    response = requests.get(f"http://<your_host>:<port>/blick/phases/{phases}")
    print(response.json())
    """
    prep_rules()
    phases = [phase.strip() for phase in phases.replace(" ", "").split(",")]
    matched_funcs = [func for func in __blick_checker.collected if func.phase in phases]
