...
from .blick_util import str_to_bool
...
def _get_sheet(wb, sheet_name=None):
    ...
    raise BlickException('A sheet name was not specified and sheet1 could not be found.')
...
def _ensure_row_params(row_end, row_start: int):
    ...
    try:
        ...
        raise BlickException("row_end was not a valid integer value") from vex
...
def rule_xlsx_a1_pass_fail(wb: openpyxl.workbook.Workbook,
    ...
    for row in range(row_start, row_end + 1):
        ...
        if value is None:
            raise BlickException(f'Expected boolean value in row {row}')
        ...
        if str_to_bool(value):
            yield BR(status=True, msg=f"{BM.expected(desc)}-Passed")
        else:
            yield BR(status=False, msg=f"{BM.expected(desc)}-Failed")
...
def rule_xlsx_df_pass_fail(df: pd.DataFrame, desc_col: str, val_col: str, skip_on_none=False):
    ...
    for row in df.values:
        ...
        if pd.isnull(row_dict[val_col]):
            if skip_on_none:
                yield BR(status=None, skipped=True,
        ...
        if status:
            yield BR(status=True, msg=f"{BM.code(description)}-Passed")
        else:
            yield BR(status=False, msg=f"{BM.code(description)}-Failed")
