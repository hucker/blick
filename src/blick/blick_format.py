...
In order to accomplish this, the BlickMarkup class allows a very simple formatting of the messages.  THis
...
You can just write the markdown in your messages, or you can use the BlickMarkup class to generate
the inline text. 
...
class BlickMarkup:
    ...
    Blick messages can have embedded formatting information that indicates to higher level
...
class BlickRenderText(BlickAbstractRender):
    """
...
class BlickBasicMarkdown(BlickRenderText):
...
class BlickBasicRichRenderer(BlickRenderText):
    """Rich render class"""
...
class BlickBasicStreamlitRenderer(BlickRenderText):
...
class BlickBasicHTMLRenderer(BlickRenderText):
    """HTML renderer"""
