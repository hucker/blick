"""
This module contains the `BlickPackage` class, which provides the functionality to load Blick
modules from a specified directory, manage these modules, and retrieve their properties.

The `BlickPackage` class provides the following main functionalities:
...
class BlickPackage:
    ...
    def load_modules(self, glob=None) -> list[BlickModule]:
        ...
        for file_path in sorted(self.folder.glob(check_file_glob)):
            ...
            module = BlickModule(module_name, module_file=str(file_path), auto_load=True)
            self.modules.append(module)

        return self.modules

    def get(self, module_name) -> BlickModule | None:
