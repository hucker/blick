...
def extract_tables_from_pdf(file_path: str,
    ...
    try:
        ...
        raise BlickException(f"IO Error reading PDF file {file_path}.") from ioex
    except Exception as exc:
        raise BlickException(f"Error extracting tables from PDF: {file_path}") from exc
...
def rule_from_pdf_rule_ids(file_path: str,
                           ...
                           max_results: int = 1,
                           pages="all") -> Generator[BR, None, None]:
    """
        ...
        -------
        namedtuple
           Namedtuple represented by SR where:
    ...
    for table in tables:
        if rule_col in table.columns:
            ...
            for _, rule_data in df.iterrows():
                ...
                yield BR(status=status, msg=msg, skipped=skip)
                count += 1
                if count > max_results:
                    raise BlickException(f"Maximum number of results ({max_results}) exceeded for {file_path}")
    if count == 0:
        yield BR(status=None, skipped=True, msg=f"No results found in {file_path}")
