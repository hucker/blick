...
from . import BlickException
from .blick_format import BM
from .blick_result import BR, BlickYield
...
def rule_url_200(urls: str | Sequence[str],
                 ...
                 timeout_sec=5,
                 summary_only=False,
                 summary_name=None) -> Generator[BR, None, None]:
    """
    ...
    """
    y = BlickYield(summary_only=summary_only, summary_name=summary_name or "Rule 200 Check")
...
def rule_web_api(url: str,
    ...
    y = BlickYield(summary_only=summary_only)
    try:
        ...
        if isinstance(expected_response,int):
            ...
            raise BlickException(f"Expected integer or list of integers for 'expected_response' for {url}")
