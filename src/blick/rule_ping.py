...
def rule_ping_check(hosts: str | list, timeout_ms: float = 4000.0, skip_on_none=False, pass_on_none=False) -> Generator[
    ...
    if len(hosts) == 0:
        if skip_on_none:
            yield BR(status=True, skipped=True, msg=NO_HOSTS_MSG)
            return
        else:
            yield BR(status=pass_on_none, msg=NO_HOSTS_MSG)
            return
    ...
    for host in hosts:
        try:
            ...
            if latency is False:
                ...
                yield BR(status=True, msg=f"Host {BM.code(host)} is up: response time < {BM.code(latency_str)} ms")
            else:
                latency_str = f"{latency:0.1f}"
                yield BR(status=True, msg=f"Host {BM.code(host)} is up, response time = {BM.code(latency_str)} ms")
        except Exception:
            # Document your exception handling logic here
            yield BR(status=False, msg=f"Host {BM.code(host)} not found: {BM.fail(host)}")
