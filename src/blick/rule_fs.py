...
def rule_fs_paths_exist(fs_obj: OSFS, paths: Sequence[str]) -> Generator[BR, None, None]:
    """ Check a bunch of paths."""
    for path in paths:
        yield from rule_fs_path_exists(fs_obj, path)


def rule_fs_path_exists(fs_obj: OSFS, path_: str) -> Generator[BR, None, None]:
    """Simple rule to check for a file path."""
    yield BR(status=fs_obj.exists(path_), msg=f"The path {path_} on {fs_obj.root_path} exists.")
...
def rule_fs_file_within_max_size(filesys: OSFS,
    ...
    if not filesys.isfile(path):
        yield BR(status=False, msg=f'File "{path}" does {BM.bold("NOT")} exist in {filesys.root_path}', skipped=skip_if_missing)
    else:
        ...
        if delta < 0:
            yield BR(status=False, msg=f'File "{path}" size={file_size_str} exceeds size limit by -{delta} bytes.')
        else:
            yield BR(status=True, msg=f'File "{path}" size={file_size_str} within size limit by {delta} bytes.')
...
def rule_fs_oldest_file_age(filesys: FS, max_age_minutes: float = 0,
    ...
    try:
        ...
        yield BR(status=False, msg=f"Error during listing files: {str(e)}", except_=e)
        return

    if not files:
        yield BR(status=no_files_stat,
    ...
    try:
        ...
        yield BR(status=False, msg=f"Error during checking file's age: {str(e)}", except_=e)
        return
