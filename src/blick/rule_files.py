"""
This module contains blick rules that are useful for checking the status of files
...
from .blick_result import BR, BlickYield,BlickResult


def rule_path_exists(path_: str) -> Generator[BR, None, None]:
    """Simple rule to check for a file path."""
    path_str = BM.code(path_)
    try:
        if pathlib.Path(path_).exists():
            yield BR(status=True, msg=f"The path {BM.code(path_str)} does exist.")
        else:
            yield BR(status=False, msg=f"The path  {BM.code(path_str)} does {BM.bold('NOT')} exist.")
...
def rule_paths_exist(paths:list[str]|str,
                     ...
                     summary_name=None,
                     name = "Path Check",
                     no_paths_pass_status=False)->Generator[BR, None, None]:
    y=BlickYield(summary_only=summary_only, summary_name=summary_name)
...
def rule_stale_file(
    ...
    if age_in_seconds <= 0:
        raise BlickException(f"Age for stale file check {BM.code(age_in_seconds)} should be > 0")
    ...
    try:
        ...
        if file_age_in_seconds > age_in_seconds:
            ...
            if days > 0:
                ...
                raise BlickException("No file age times were specified.")
            age_msg = f"age = {file_age:.2f} {unit} {age_in_seconds=}"
            yield BR(status=False, msg=f"Stale file {BM.code(filepath)} {BM.code(age_msg)}")
        else:
            yield BR(status=True, msg=f"Not stale file {BM.code(filepath)}")
...
def rule_stale_files(
    ...
    """
    y = BlickYield(summary_only=summary_only, summary_name=summary_name or "Rule_stale_files")
...
def rule_large_files(folder: str,
    ...
    y = BlickYield(summary_only=summary_only, summary_name=summary_name or "Rule_large_files")

    if max_size <= 0:
        raise BlickException(f"Size for large file check should be > 0 not {max_size=}")
...
def rule_max_files(folders: list,
    ...
    """
    y = BlickYield(summary_only=summary_only, summary_name=summary_name or "Rule_max_files")
    ...
    if len(folders) != len(max_files):
        raise BlickException(f"Number of folders and max_files {max_files} must be the same.")
