...
def rule_validate_ndf_schema(df: FrameT,
                             ...
                             name=None)-> Generator[BR, None, None]:
    ...
    if not any((int_cols, float_cols, no_null_cols, str_cols, number_cols)):
        raise BlickException(f"No schema checks were specified for {name}")

    schema = df.dtypes

    y = BlickYield(summary_name=summary_name, summary_only=summary_only)
    ...
    for int_col in int_cols:
        if schema[int_col].name in int_types:
            yield from y(BR(True, f"Column {int_col} is an integer type"))
        else:
            yield from y(BR(False, f"Column {BM.code(int_col)} is {BM.bold('NOT')} an integer type"))

    for float_col in float_cols:
        if schema[float_col].name in float_types:
            yield from y(BR(True, f"Column {BM.code(float_col)} is a float type"))
        else:
            yield from y(BR(False, f"Column {BM.code(float_col)} is {BM.bold('NOT')} an float type"))

    for number_col in number_cols:
        if schema[number_col].name in num_types:
            yield from y(BR(True, f"Column {BM.code(number_col)} is a number type"))
        else:
            yield from y(BR(False, f"Column {BM.code(number_col)} is {BM.bold('NOT')} an number type"))

    for str_col in str_cols:
        if schema[str_col].name in str_types:
            yield from y(BR(True, f"Column {BM.code(str_col)} is an string type"))
        else:
            yield from y(BR(False, f"Column {BM.code(str_col)} is {BM.bold('NOT')} an string type"))

    for no_null_col in no_null_cols:
        null_count = len(v for v in df[no_null_col] if v is None or v == '')
        if null_count == 0:
            yield from y(BR(True, f"Column {BM.code(no_null_col)} has {null_count} empty/null values."))
        else:
            yield from y(BR(False, f"Column {BM.code(no_null_col)} has {null_count} empty/null values"))
...
def rule_ndf_columns_check(name: str, df: FrameT, expected_cols_: str | list[str], exact=False):
    ...
    Returns:
        A generator that yields BlickResult object(s) based on the column checks.
    """
    
    if df.is_empty() or not df.columns:
        if not expected_cols_:
            yield BR(status=True, msg=f"The {BM.code(name)} data frame is empty and there are no expected columns.")
        else:
...
def rule_validate_ndf_values_by_col(df: FrameT,
                                    ...
                                    name:str='')-> Generator[BR, None, None]:
    """
        ...
        Raises:
        - BlickException: If df is None, min_rows/max_rows aren't positive integers, or
    ...
    if df is None or df.is_empty():
        raise BlickException("Dataframe is empty or None")
    ...
    if not any(conditions):
        raise BlickException("No data frame column value rules specified.")

    if positive:
        for col in positive:
            if df[col].min() > 0.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are positive.")

            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')}  positive.")

    if non_negative:
        for col in non_negative:
            if df[col].min() >= 0.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are non-negative.")
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} non-negative.")

    if percent:
        # Just check 0-100
        for col in percent:
            if 0.0 <= df[col].min() and df[col].max() <= 100.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are  a percent.")
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} a percent.")

    if probability:
        for col in probability:
            # Just check 0-100
            if 0.0 <= df[col].min() and df[col].max() <= 1.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are probabilities.", )
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} probabilities.")

    if correlation:
        for col in correlation:
            # Just check 0-100
            if -1.0 <= df[col].min() and df[col].max() <= 1.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are correlations.", )
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} correlations.")

    if min_:
        val, cols = min_
        for col in cols:
            if df[col].min() >= val:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are > {val}")
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} > {val}")
    ...
    if negative:
        for col in negative:
            if df[col].max() < 0.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are negative.")
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} negative.")

    if non_positive:
        for col in non_positive:
            if df[col].max() <= 0.0:
                yield BR(status=True, msg=f"All values in {BM.code(col)} are non-positive.")
            else:
                yield BR(status=False, msg=f"All values in {BM.code(col)} are {BM.bold('NOT')} non-positive.")
    ...
    if non_positive:
        for col in non_positive:
            if df[col].max() < 0:
                ...
                yield BR(status=False, msg=f"All values in {col} are {BM.bold('NOT')} non-positive.")
...
def rule_ndf_pf_columns(df: FrameT,
                        ...
                        name: str = '',
                        summary_only: bool = False) -> Generator[BR, None, None]:
    """
    ...
    summary_yield = BlickYield(summary_only=summary_only)
