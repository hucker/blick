""" Test the pre- and post-hooks for the BlickResult"""
...
def test_result_pre_hook_result_only():
    ...
    def hook_test():
        ...
        yield blick.BlickResult(status=True, msg="It works")
    ...
    sp_func = blick.BlickFunction(hook_test, pre_sr_hooks=[result_hook1])
    for result in sp_func():
        assert result.status is False
        assert result.msg == "It works"

    sp_func = blick.BlickFunction(hook_test, pre_sr_hooks=[result_hook2])
    for result in sp_func():
...
def test_result_pre_hook_use_class():
    """ Test that the pre hook can use data from the BlickFunction class"""
    ...
    def result_doc_string_to_msg(self: blick.BlickFunction, sr):
        """ Hook to change the doc string to the msg"""
        sr.msg = self.doc
        return sr

    sp_func = blick.BlickFunction(hook_test, pre_sr_hooks=[result_doc_string_to_msg])
    for result in sp_func():
...
def test_verify_post_changes_pre():
    ...
    sp_func = blick.BlickFunction(hook_test, pre_sr_hooks=[result_pre], post_sr_hooks=[result_post])
    for result in sp_func():
