...
@pytest.mark.parametrize(
    ...
    [
        (
                ['id', 'name', 'email', 'age'],
                [
                    blick.BlickResult(status=True, msg="Column <<code>>id<</code>> is present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>name<</code>> is present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>email<</code>> is present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>age<</code>> is present in table <<code>>users<</code>>")
                ]
        ),
        (
                ...
                [
                    ...
                    blick.BlickResult(status=True, msg="Column <<code>>email<</code>> is present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>age<</code>> is present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=False, msg="Column <<code>>unexpected<</code>> is <<fail>>MISSING<</fail>> in table <<code>>users<</code>>")
                ]
        ),
    ],
)
...
def test_rule_sql_table_schema(engine, expected_columns, expected_results):
    ...
    results = list(blick.rule_sql_table_col_name_schema(engine, 'users', expected_columns))
...
@pytest.mark.parametrize(
    ...
    [
        (
                ['name', 'email', 'age'],
                [
                    blick.BlickResult(status=True, msg="Column <<code>>name<</code>> is correctly present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>email<</code>> is correctly present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>age<</code>> is correctly present in table <<code>>users<</code>>")
                ]
        ),
        (
                ['id', 'name', 'email'],
                [
                    blick.BlickResult(status=True, msg="Column <<code>>id<</code>> is correctly present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>name<</code>> is correctly present in table <<code>>users<</code>>"),
                    blick.BlickResult(status=True, msg="Column <<code>>email<</code>> is correctly present in table <<code>>users<</code>>"),
                ]
        ),
...
def test_rule_sql_table_schema_with_extra(engine, expected_columns, expected_results):
    # Call the function with the expected columns list
    results = list(blick.rule_sql_table_col_name_schema(engine, 'users', expected_columns, extra_columns_ok=True))
...
def test_rule_sql_table_bad_table(engine):
    # Call the function with the expected columns list
    results = list(
        blick.rule_sql_table_col_name_schema(engine, table='', expected_columns=['users'], extra_columns_ok=True))
...
def test_rule_sql_table_bad_column(engine):
    # Call the function with the expected columns list
    results = list(
        blick.rule_sql_table_col_name_schema(engine, table='users', expected_columns=[''], extra_columns_ok=True))
...
def test_rule_sql_table_bad_extra_columns(engine):
    # Call the function with the expected columns list
    results = list(blick.rule_sql_table_col_name_schema(engine, table='users', expected_columns=['id', 'name'],
...
def test_rule_sql_table_types(engine):
    results = list(blick.rule_sql_table_schema(engine,
                                                table='users',
...
def test_rule_sql_all(engine_alltypes):
    ...
    results = list(blick.rule_sql_table_schema(engine_alltypes,
                                                table='users',
                                                expected_columns=[('id', INTEGER()),
                                                                  ('name', VARCHAR()),
                                                                  ('age', DOUBLE()),
