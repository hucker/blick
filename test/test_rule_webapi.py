...
from src.blick.rule_webapi import is_mismatch
...
def test_urls():
    ...
    def check_rule1():
        yield from blick.rule_url_200(urls=urls)
...
def test_urls_summary_only():
    ...
    @blick.attributes(tag="tag")
    def check_rule1():
        yield from blick.rule_url_200(urls=urls, summary_only=True)
...
def test_missing_web_api():
    ...
    @blick.attributes(tag="tag")
    def check_rule2():
        yield from blick.rule_web_api(url='https://httpbin.org/json1',
...
def test_wrong_response(expected_json):
    @blick.attributes(tag="tag")
    def check_rule1():
        yield from blick.rule_web_api(url='https://httpbin.org/json',
