...
def test_ttl_func(tmp_path):
    ...
    @blick.attributes(ttl_minutes='1s')
    def func():
        yield blick.BR(status=f.exists(), msg="Exist 1")
        yield blick.BR(status=f.exists(), msg="Exist 2")
        yield blick.BR(status=f.exists(), msg="Exist 3")

    blick_func = blick.BlickFunction(func, "")

    ch = blick.BlickChecker(check_functions=[blick_func], auto_setup=True)
    results = ch.run_all()
