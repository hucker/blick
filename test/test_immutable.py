...
The expected outcome of these operations is the raise of BlickException, indicating successful prevention of
...
from src.blick import BlickException
...
def env_list():
    return blick.BlickEnvList([1, 2, 3, 4, 5])


def test_list_setitem(env_list):
    with pytest.raises(BlickException):
        env_list[0] = 100
...
@pytest.fixture
def env_dict():
    return blick.BlickEnvDict({"a": 1, "b": 2, "c": 3})
...
def func_dict():
    ...
    return blick.BlickFunction(func_dict)


@pytest.fixture
def func_set():
    @blick.attributes(tag="env_set")
...
def test_blick_function_writing_to_env_dict(func_dict):
    env = {'env_list': [1, 2, 3], 'env_dict': {'a': 10, 'b': 11}, 'env_set': {1, 2, 3}}

    ch = blick.BlickChecker(check_functions=[func_dict], env=env, auto_setup=True)
    results = ch.run_all()
    assert len(results) == 1
    assert results[0].except_


def test_blick_function_writing_to_env_list(func_list):
    env = {'env_list': [1, 2, 3], 'env_dict': {'a': 10, 'b': 11}, 'env_set': {1, 2, 3}}
    ch = blick.BlickChecker(check_functions=[func_list], env=env, auto_setup=True)
    results = ch.run_all()
    assert len(results) == 1
    assert results[0].except_


def test_blick_function_writing_to_env_set(func_set):
    env = {'env_list': [1, 2, 3], 'env_dict': {'a': 10, 'b': 11}, 'env_set': {1, 2, 3}}
    ch = blick.BlickChecker(check_functions=[func_set], env=env, auto_setup=True)
    results = ch.run_all()
    assert len(results) == 1
    assert results[0].except_
