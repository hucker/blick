...
blick_rc code and then  the dictionaries used to initialize the
BlickRC class should be written off to JSON and TOML and then the
...
def test_simple_summary(test_data):
    rc = blick_rc.BlickRC(rc_d=test_data)
...
def test_bad_rcd(bad_type):
    with pytest.raises(blick.BlickException) as exec_info:
        _ = blick_rc.BlickRC(rc_d=bad_type)
...
def test_regex_rc(test_data):
    rc = blick_rc.BlickRC(rc_d={'ruids': 'r.*'})

    assert rc.does_match(ruid='r1')
    assert rc.does_match(ruid='r2')
    assert rc.does_match(ruid='rasdfasdfasdf')

    rc = blick_rc.BlickRC(rc_d={'ruids': r'r\d', 'phases': r'p\d'})
...
def test_regex_fixture_rc(rules, ruid, phase, tag, expected):
    rc = blick_rc.BlickRC(rc_d=rules)
...
def test_neg():
    rc = blick_rc.BlickRC(rc_d={'tags': ['t1', '-t2'],
                                  'ruids': ['r1', '-r2'],
                                  'phases': 'p1,-p2'})
