...
@pytest.fixture
def simple1_pkg():
    return blick.BlickPackage(folder="./simple1", name="Simple1")


@pytest.fixture
def decorator1_pkg():
    return blick.BlickPackage(folder="./decorator1", name="Decorator1")


@pytest.fixture
def decorator2_pkg():
    return blick.BlickPackage(folder="./decorator2", name="Decorator2")


@pytest.fixture
def skip_pkg():
    return blick.BlickPackage(folder="./skip", name="Skip")


@pytest.fixture
def pkg_2_modules():
    return blick.BlickPackage(folder="./ruid")


@pytest.fixture
def skip_no_name_pkg():
    return blick.BlickPackage(folder="./skip")


def test_index_in_modules(skip_pkg):
    """ Verify that packages read from modules have the check functions indexed correctly """
    ch = blick.BlickChecker(packages=[skip_pkg], auto_setup=True)
...
def test_no_package():
    with pytest.raises(blick.BlickException):
        _ = blick.BlickPackage(folder="__non_existent_folder")


def test_sugar():
    r1 = blick.BlickResult(status=True, msg="msg")
    r2 = blick.BR(status=True, msg="msg")
    assert r1 == r2
