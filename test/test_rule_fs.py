...
import blick.rule_fs as rule_fs
...
def test_fs_fixture_exists(temp_fs: FS):
    ...
    def check_temp_file(filesys: FS):
        yield blick.BR(status=filesys.exists('test.txt'), msg="Check if file exists")

    sfunc = blick.BlickFunction(check_temp_file)
    chk = blick.BlickChecker(check_functions=[sfunc], env={'filesys': temp_fs}, auto_setup=True)
...
def test_fs_rule_files_exist(temp_fs: FS):
    ...
    sfunc = blick.BlickFunction(check_temp_files)
    chk = blick.BlickChecker(check_functions=[sfunc], env={'filesys': temp_fs}, auto_setup=True)
    results = chk.run_all()
    assert results[0].status is True
    assert results[1].status is False
