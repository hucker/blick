...
def check_global(global_env):
    ...
    yield blick.BlickResult(status=global_env == "hello", msg=f"Got global env={global_env}")


def check_env1(db_config):
    """Should pick up db config from local env"""
    yield blick.BlickResult(status=db_config == CONST_DB_CONFIG, msg="DB config is correct")


def check_env2(number_config):
    """Should pick up number_config from local_env"""
    yield blick.BlickResult(status=number_config == CONST_NUMBER_CONFIG,
                             msg=f"Numeric config is correct {number_config}")


def check_env3(data_frame):
    """ Should pickup dataframe from local config"""
    yield blick.BlickResult(status=isinstance(data_frame, pd.DataFrame), msg="Data_frame is actually a dataframe.")
    yield blick.BlickResult(status=data_frame.empty is True, msg="Dataframe is empty")
