...
def test_rule_file_exist():
    ...
    def check_rule1():
        for result_ in blick.rule_path_exists(path_="./rule_files_/my_file.txt"):
            yield result_

    @blick.attributes(tag="tag")
    def check_rule2():
        yield from blick.rule_path_exists(path_="./rule_files_/my_file.dummy")

    s_func1 = blick.BlickFunction(check_rule1, '')
    s_func2 = blick.BlickFunction(check_rule2, '')
...
def test_rule_files_exist():
    ...
    @blick.attributes(tag="tag")
    def check_rule1():
        yield from blick.rule_paths_exist(paths="./rule_files_/my_file.txt ./rule_files_/my_file.txt")
...
def test_rule_large_files():
    ...
    @blick.attributes(tag="tag")
    def check_rule1():
        for result_ in blick.rule_large_files(folder="./rule_files_", pattern="*.txt", max_size=10000):
            yield result_

    @blick.attributes(tag="tag")
    def check_rule2():
        for result_ in blick.rule_large_files(folder="./rule_files_", pattern="*.txt", max_size=50):
            yield result_
...
def test_rule_large_file_bad_setup():
    def check_rule_bad_setup():
        yield from blick.rule_large_files(folder="./rule_files_", 
                                           pattern="*.foobar", 
                                           max_size=-50,
                                           no_files_pass_status=True)

    s_func1 = blick.BlickFunction(check_rule_bad_setup)
    for result in s_func1():
        assert result.status is False
...
def test_rule_large_files_missing():
    ...
    s_func1 = blick.BlickFunction(check_rule_missing_pass)
    for result in s_func1():
        assert result.status

    s_func2 = blick.BlickFunction(check_rule_missing_fail)
    for result in s_func2():
        assert result.status is False
...
def test_bad_stale_file_setup(days, hours, minutes, seconds):
    ...
    def check_rule_files():
        yield from blick.rule_stale_files(folder=file_path, pattern="my_file*.txt", days=days, hours=hours,
                                           minutes=minutes, seconds=seconds)

    s_func = blick.BlickFunction(check_rule_files)
...
def test_stale_file_no_match():
    ...
    def check_rule_missing_true():
        yield from blick.rule_stale_files(folder=file_path, pattern="my_file*.foobar", days=0, hours=0, minutes=0,
    ...
    s_func1 = blick.BlickFunction(check_rule_missing_true)
    for result in s_func1():
        assert result.status

    s_func2 = blick.BlickFunction(check_rule_missing_false)
    for result in s_func2():
...
def test_stale_files():
    ...
    @blick.attributes(tag="tag")
    def check_rule_sec():
        yield from blick.rule_stale_files(folder=file_path, 
                                           pattern="my_file*.txt", 
                                           days=0, 
                                           hours=0, 
                                           minutes=0,
    ...
    for rule in [check_rule_sec]:#, check_rule_min, check_rule_hour, check_rule_day]:
        ...
        s_func1 = blick.BlickFunction(rule, '')
        for result in s_func1():
            assert result.status
...
def test_stale_file_summary():
    ...
    @blick.attributes(tag="tag")
    def check_rule_sec():
        yield from blick.rule_stale_files(folder=file_path, pattern="my_file*.txt", days=0, hours=0, minutes=0,
                                           seconds=.5,summary_only=True,summary_name="Rule_files_ stale check")

    sfunc = blick.BlickFunction(check_rule_sec)
    # Touch the file
    pathlib.Path("./rule_files_/my_file.txt").touch()
...
def test_check_rule(test_name, folder, max_files, expected_status):
    @blick.attributes(tag="tag")
    def check_rule():
        yield from blick.rule_max_files(folders=folder, max_files=max_files)
...
def test_bad_max_files():
    def check_rule1():
        yield from blick.rule_max_files(folders=["./rule_files_"], max_files=[10, 20, 30])

    sfunc = blick.BlickFunction(check_rule1)
    results = list(sfunc())
...
def test_max_file_summary():
    @blick.attributes(tag="tag")
    def check_rule():
        yield from blick.rule_max_files(folders='./rule_files_',
                                         max_files=10,
                                         summary_only=True,
