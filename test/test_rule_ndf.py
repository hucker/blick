...
df libraries work, but that the blick interface to narwhals is working with the ideal being that
blick has NO dependency on dataframe libraries and only depends upon narwhals.
...
def test_rule_ndf(dataframe: pd.DataFrame):
    ...
    s_func1 = BlickFunction(check_good_narwhals_frame)
...
def test_rule_ndf_unexpected_null(null_dataframe: pd.DataFrame):
    ...
    s_func1 = BlickFunction(check_null_narwhals_frame)
    for result in s_func1():
        assert result.status is False
...
def test_rule_ndf_pf_columns_enabled(request, df_fixture_name, expected_status, expected_length):
    ...
    s_func1 = BlickFunction(check_narwhals_pf_frame)
